...
url_filter = init_static_bloom(500000)  # 初始化url过滤器 debug的时候要用定长，否则停不下来，也无法检验函数作用
...
def start_threads(url_queue, html_queue, thread_num=100):
    '''
    启动线程函数，并返回线程列表
